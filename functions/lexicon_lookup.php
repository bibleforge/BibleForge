...
function retrieve_lexical_data($word_id, $language)
{
    ...
    if ($word_id < 621740) {
        $SQL_query = 'SELECT `bible_original`.word, `bible_original`.pronun, `lexicon_hebrew`.strongs, `lexicon_hebrew`.base_word, `lexicon_hebrew`.data, `lexicon_hebrew`.usage FROM `bible_' . $language['identifier'] . '`, `bible_original`, `lexicon_hebrew`, `morphology` WHERE `bible_' . $language['identifier'] . '`.id = ' . ((int)$word_id) . ' AND `bible_original`.id = `bible_' . $language['identifier'] . '`.orig_id AND lexicon_hebrew.strongs = `bible_original`.strongs LIMIT 1';
    } else {
        $SQL_query = 'SELECT `bible_original`.word, `bible_original`.pronun, `lexicon_greek`.strongs, `lexicon_greek`.base_word, `lexicon_greek`.data, `lexicon_greek`.usage, `morphology`.part_of_speech, `morphology`.declinability, `morphology`.case_5, `morphology`.number, `morphology`.gender, `morphology`.degree, `morphology`.tense, `morphology`.voice, `morphology`.mood, `morphology`.person, `morphology`.middle, `morphology`.transitivity, `morphology`.miscellaneous, `morphology`.noun_type, `morphology`.numerical, `morphology`.form, `morphology`.dialect, `morphology`.type, `morphology`.pronoun_type FROM `bible_' . $language['identifier'] . '`, `bible_original`, `lexicon_greek`, `morphology` WHERE `bible_' . $language['identifier'] . '`.id = ' . ((int)$word_id) . ' AND `bible_original`.id = `bible_' . $language['identifier'] . '`.orig_id AND lexicon_greek.strongs = `bible_original`.strongs AND `morphology`.id = `bible_original`.id LIMIT 1';
    }
